...
\author{Thomas Wisson\\
\texttt{thomas.wisson@etudiant.univ-bpclermont.fr}}
\date{May and June 2011}
\begin{document}
\maketitle


%\include(blabla.tex) Rajoute tout le blabla utile pour le rapport de stage
...
Your first idea is to use the algorithm based on the dynamic programming 
...
\subsubsection{The model}\label{sbsec:model1}
\begin{align}
...
\subsubsection{The planning horizon}
...
\subsubsection{Idea to perform the algorithm}
...
Whitin \cite{Wagner&Whitin58} they compare different slope of this function g. 
This lead to a $logT$ complexity.
...
Thomas's algorithm \cite{Thomas70} and of course is the prices are not constant
...
the price. Thus we can't compare slope of the function and consequently use the 
...
like sale moreover marketing is really part of commercial strategy, that's we
...
\begin{align}
    ...
    X_t\\
    \intertext{subject to}
    &d(p_t,q_t)= \kappa \frac{q_t^{\gamma}}{{p_t} ^{\alpha}}\\
    &X_t + I_{t-1} - I_t= d_t\\
    &I_t \geq 0\;\;\; X_t \geq 0\;\;\;\delta_t\in \{0,1\}\notag
\end{align}

%TODO Add demand curve
...
We want to have a link between the demand and the marketing, do to that we set an
continuous variable of marketing, a marketing by unit which allow to insert it in 
the demand. 
